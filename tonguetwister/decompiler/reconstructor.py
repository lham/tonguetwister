...
def ensure(var, klass, msg=''):
  if isinstance(klass, list):
    if not any(map(lambda k: isinstance(var, k), klass)):
      msg = (
        'Expected: %s\n'
        'Actual:   %s\n'
        '%s\n'
        ) % (', '.join(map(lambda k: k.__name__, klass)), var.__class__.__name__, msg)
      raise UnexpectedOperatorException(var, msg)
  else:
    if not isinstance(var, klass):
      msg = (
        'Expected: %s\n'
        'Actual:   %s\n'
        '%s\n'
      ) % (klass.__name__, var.__class__.__name__, msg)
      raise UnexpectedOperatorException(var, msg)
...
class UnexpectedOperatorException(Exception):
  def __init__(self, op, msg = None):
    if not msg:
      ...
      self.message = '\n' + str(msg)
    super(UnexpectedOperatorException, self).__init__(self.message)

class OperatorNotFoundException(Exception):
  pass

class OperatorSequence(object):
  ...
  def find(self, klass, start=0):
    """
    Find the first occurrence of an operator of a specified class.

    Args:
      op_klass: Class of the operator to be found.
  ...
  def find_all(self, klass, start=0):
    """
    Find all the occurrences of an operator of a specified class.

    Args:
      op_klass: Class of the operators to be found.
      start: Offset from which the search will be performed.
    Returns:
      An iteratable of all the operators with class `op_klass`, if no such operator is found 
      `None` is returned.
    """
    while True:
      pos = self.find(klass, start)
      if not pos:
        break
      yield pos
...
def parse_iterator_cleanups(opseq):
  for pos in opseq.find_all(CleanupIteratorOperator):
    ...
    if cleaned != iterations:
      raise Exception('Could not clean as many push values as stated.')
...
def parse_conditional_statements(opseq):
  for if_pos in opseq.find_all(IfTrueOperator):
    ...
    # We might have extracted the target address already, thus we need to do some loookup tricks
    add_one = False
    while not jmp_pos:
      addr -= 1
      jmp_pos = opseq.find_by_address(addr)
    ...
    elif isinstance(jmp_target_prev_op, IfTrueOperator) and isinstance(opseq.peek(opseq.find_by_address(jmp_target_prev_op.jmp_addr)), IteratorOperator):
      # This is a switch-statement
      parse_single_switch_conditional(opseq, if_pos)
...
def parse_single_loop_conditional(opseq, pos):
  ...
  end_repeat_op = opseq.extract(end_repeat_pos)
  ...
  # Insert loop ending staement
  end_repeat_stmt = LoopEndStatement()
  opseq.insert(end_repeat_pos, end_repeat_stmt)

def parse_single_if_conditional(opseq, ifpos):
  """
  """
  ifop = opseq.extract(ifpos)

  # Parse the condition
  cond = None
  if isinstance(ifop.comparison, Variable):
    ifpos, expr = extract_variable(opseq, ifpos - 1)
    cond = FunctionCallArgument(expr.__class__.__name__, expr)
  elif isinstance(ifop.comparison, BinaryResult):
    ifpos, cond = extract_binary_result(opseq, ifpos - 1)
  elif isinstance(ifop.comparison, UnaryResult):
    ifpos, cond = extract_unary_result(opseq, ifpos - 1)
  elif isinstance(ifop.comparison, FunctionResult):
    ifpos, cond = extract_function(opseq, ifpos - 1)
  elif isinstance(ifop.comparison, ObjectProperty):
    ifpos, cond = extract_object_property(opseq, ifpos - 1)
  else:
    raise UnexpectedOperatorException(ifop.comparison)

  # Try to find an else-statement
  addr = ifop.jmp_addr
  endop_pos = opseq.find_by_address(addr)

  # We might have extracted the target address, thus we need to do some loookup tricks
  add_one_due_to_missing_endop = False
  while not endop_pos:
    addr -= 1
    endop_pos = opseq.find_by_address(addr)
    add_one_due_to_missing_endop = True

  if add_one_due_to_missing_endop:
    endop_pos += 1

  endop = opseq.peek(endop_pos - 1)

  if isinstance(endop, SwitchEndStatement):
    endop_pos -= 1
    endop = opseq.peek(endop_pos - 1)

  if isinstance(endop, ElseOperator): # if-statement has else-clause
    elseop_pos = endop_pos - 1
    elseop = opseq.extract(elseop_pos)
    elseop_target_addr = elseop.jmp_addr

    endop_pos = opseq.find_by_address(elseop_target_addr)
    endop = opseq.peek(endop_pos - 1)

    ending_op = IfEndStatement()
    opseq.insert(endop_pos, ending_op)

    elsing_op = ElseControlStatement()
    opseq.insert(elseop_pos, elsing_op)

    opening_op = IfControlStatement(cond)
    opseq.insert(ifpos, opening_op)
  else:
    ending_op = IfEndStatement()
    opseq.insert(endop_pos, ending_op)
...
def extract_switch_case(opseq, pos, switch_cond_val):
  ...
  if cond.op == '!=':
    ...
    if isinstance(jmp_target_op, CleanupIteratorOperator):
      ...
      raise UnexpectedOperatorException()

  else:
    raise Exception('Wrong condition in switch statement')
...
def extract_argument_list(opseq, pos, arg_list_op=None):
  ...
  while i < len(args_to_process):
    ...
    ensure(push_op, [PushOperator, IteratorOperator, BinaryOperator, PushObjectPropertyOperator, FunctionCall, CallFunctionOperator])
...
def extract_binary_result(opseq, pos, binary_instruction=None):
  ...
  Args:
    ...
    binary_instruction: The instruction of the binary result if the operator has already been extrated.
  Returns:
